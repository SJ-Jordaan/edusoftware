...
export function DFA(alphabet, delta, initial, final) {
...
DFA.prototype.test = function (str) {
  // alias for accepts, to match JS regex
...
DFA.prototype.without_unreachables = function () {
  // todo naming conventions
  /*  non-destructively produce equivalent DFA without unreachable states */
  var reached = {};
  crawl(this.alphabet, this.initial, this.delta, {
    state_fn: function (s) {
      reached[s] = true;
    },
  });
...
DFA.prototype.find_passing = function () {
  ...
  return crawl(this.alphabet, this.initial, this.delta, {
    should_exit: function (s) {
      return this.final.indexOf(s) !== -1;
    }.bind(this),
  });
};
...
DFA.prototype.intersect = function (other) {
  ...
  function state_fn(state) {
    ...
    newDelta[curName] = {};
    if (
      thiz.final.indexOf(state[0]) !== -1 &&
      other.final.indexOf(state[1]) !== -1
    ) {
      newFinal.push(curName);
    }
  }

  crawl(this.alphabet, initial, follow, {
    get_name: get_name,
    state_fn: state_fn,
  });
  ...
  this.states.forEach(function (state) {
    ...
    thiz.states.forEach(function (target) {
      ...
      out +=
        ...
        reprEscape(target) +
        '" [label="' +
        edges[target].map(reprEscape).join(',') +
        '"];';
    });
  });
  out += '\n}\n';
  ...
  for (var i = 0; i < newStates.length; ++i) {
    ...
    for (var j = 0; j < alphabet.length; ++j) {
      ...
      deltaStr +=
        ...
        reprEscape(sym) +
        '": "' +
        get_name(this.delta[newStates[i]][sym]) +
        '"';
    }
    deltaStr += '}';
  }
...
export function NFA(alphabet, delta, initial, final) {
  /*  alphabet is a list of characters.
...
NFA.prototype.epsilon_closure = function (states) {
  ...
  while (processing.length > 0) {
    ...
    for (var i = 0; i < next.length; ++i) {
      if (processing.indexOf(next[i]) === -1 && out.indexOf(next[i]) === -1) {
        // TODO consider alternatives to indexOf
...
NFA.prototype.step = function (states, sym) {
  ...
  states = states.reduce(function (a, b) {
    return a.concat(b);
  }); // no flatmap, so map + flatten.
...
NFA.prototype.test = function (str) {
  // alias for accepts, to match JS regex
  return this.accepts(str);
};
...
NFA.prototype.to_DFA = function () {
  ...
  function state_fn(state) {
    ...
    for (var i = 0; i < thiz.final.length; ++i) {
      if (state.indexOf(thiz.final[i]) !== -1) {
        // i.e., state contains an accepting state
        newFinal.push(curName);
        break;
      }
    }
  ...
  for (i = 0; i < this.states.length; ++i) {
    ...
    for (var j = 0; j <= this.alphabet.length; ++j) {
      ...
      for (var k = 0; k < res.length; ++k) {
        // todo three nested loops is almost certainly not the best way to do this.
        var existing = newDelta[res[k]][sym];
        newDelta[res[k]][sym] =
          existing === undefined ? [state] : deduped(existing.concat([state])); // todo could just update in-place
      }
    }
  }
...
NFA.prototype._clone = function (prefix) {
  ...
  return new NFA(
    this.alphabet,
    newDelta,
    this.initial.map(get_name),
    this.final.map(get_name),
  );
};
...
NFA.prototype.union = function (other) {
  ...
  return new NFA(
    ...
    newThis.delta,
    ['s'],
    newThis.final.concat(newThat.final),
  );
};
...
function deduped(l) {
  // non-destructively remove duplicates from list. also sorts.
  return l
...
function reprEscape(str) {
  // does not handle unicode or exceptional cases properly.
  return str
