    ...
    google: GoogleAdapter({
      ...
      onSuccess: async (tokenset) => {
        try {
          ...
          const redirect = `${process.env.IS_LOCAL ? 'http://localhost:5173' : StaticSite.LTLearn.url}/login/callback`;
          ...
          if (error instanceof UnauthorizedError) {
            const errorRedirect = `${process.env.IS_LOCAL ? 'http://localhost:5173' : StaticSite.LTLearn.url}/login/unauthorized`;
            return Session.parameter({
          ...
          const errorRedirect = `${process.env.IS_LOCAL ? 'http://localhost:5173' : StaticSite.LTLearn.url}/login/failed`;
          return Session.parameter({
            redirect: errorRedirect,
            type: 'public',
            properties: {
